...
// eslint-disable-next-line no-undef
onMounted(() => {
  const isExist = userStore.state.isAuthenticated
  if (isExist) {
    myInfoForm.year = userStore.state.user.schoolYear
    myInfoForm.semester = userStore.state.user.semester
  }
})
const confirmSetup = () => {
  // 查询是否存在该用户配置
  ...
  if (isExist) {
    //   修改已存在配置
    userStore.actions.updateInfo(myInfoForm.year, myInfoForm.semester)
