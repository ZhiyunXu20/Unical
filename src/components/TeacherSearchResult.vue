...
const buildings = computed(() =>
    Array.from(new Set(resultArr.value.map((course) => course.teacher)))
)
...
const refresh = async (val) => {
  const courses = await courseStore.allCourse();
  console.log(courses)
  resultArr.value = courses.filter(
      (course) => course.teacher.toUpperCase().includes(val.toUpperCase()) || course.teacher === val.toUpperCase()
  )
  // 初始化再次查询时候的变量值
  selectedType.value = ''
  selectedWeek.value = ''
...
const onTypeChange = (item) => {
  const filteredWeeks = Array.from(
      new Set(
          resultArr.value
              .filter((course) => course.type === item && course.teacher === selectedBuilding.value)
              .map((course) => course.week).sort((a, b) => a - b)
      )
  )
...
const onBuildingChange = (item) => {
  const types = Array.from(
      new Set(
          resultArr.value.filter((course) => course.teacher === item).map((course) => course.type)
      )
  )
  ...
  const filteredDayStartTimes = resultArr.value
      .filter(
          (course) =>
              ...
              course.week === item &&
              course.teacher === selectedBuilding.value
      )
      .map((course) => ({
...
const onDayStartTimeChange = (item) => {
  ...
  router.push({name: 'course-view', query: {id: selectedCourse.value.courseId}})
}
